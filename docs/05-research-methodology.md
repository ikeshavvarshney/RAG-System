...
This document sets out the experimental design in advance of running it. That ordering is deliberate: an evaluation designed after the results are known cannot be treated as a controlled evaluation.
...
Hybrid retrieval is widely adopted on the reasoning that dense and sparse methods fail in complementary ways. The degree to which this holds, and the balance at which it is optimal, is corpus-dependent and is often left at a library default. This question measures it directly.
...
Vision-based document extraction is more expensive than OCR by a considerable margin. Whether that cost purchases a corresponding improvement in downstream answer quality, as opposed to merely producing richer intermediate output, is an empirical question that this study addresses.
...
RAGAS employs a language model as judge. This introduces measurement noise and is acknowledged as a limitation in section 5.6. The alternative, human evaluation at sufficient scale, is not feasible within the project's timeframe.
...
These are reported alongside the quality metrics for each ablation configuration. Their role is secondary, since the research questions concern answer quality, but a cost-quality comparison materially strengthens the recommendation each result supports.
...
| **Keyword-heavy** | ~10% | Exact identifiers and rare terms; **the category in which sparse retrieval should demonstrate its value, and therefore what makes the RQ1 sweep informative rather than flat** |
| Out-of-corpus | ~5% | Should trigger web fallback or an honest non-answer; tests against fabrication |

Two constraints on the table and chart questions are essential to RQ2's validity. First, they must be genuinely unanswerable from surrounding prose: a question whose answer also appears in a caption or nearby paragraph will be answered identically under both conditions and contributes nothing. Second, they must be drawn from documents selected for containing real tables and charts, which is why corpus construction (section 5.3) treats chart density as a requirement rather than leaving it to chance.
...
**A naive baseline configuration is also evaluated:** dense-only retrieval, fixed-size chunking, no reranking, and no query expansion. This provides an absolute floor. Without it, every reported figure is relative to the project's own tuned configurations, and nothing establishes that the complete pipeline outperforms a conventional one.
...
**Validation.** An automated validator enforces document count, format coverage, the chart-density floor, manifest completeness, and manifest-to-filesystem consistency. It runs as a blocking gate rather than as an advisory check.
...
Token consumption and per-stage latency are recorded for each configuration. Fusion weighting is not expected to alter cost materially, since the same retrievers execute regardless of how their results are weighted. A substantial cost difference between configurations would therefore indicate a confound rather than a finding, and is checked for that reason.

### Anticipated Outcomes

Three outcomes are possible and all are reportable:

1. **A hybrid optimum.** Some intermediate weighting outperforms both endpoints. This is the expected result, and the one that would justify hybrid retrieval on this corpus.
...
3. **Category-dependent optimum.** Different question categories favour different weightings, which is plausible given that the keyword-heavy category is constructed to favour sparse retrieval. This would be the most informative outcome, and would support a per-query adaptive weighting proposal in future work.
...
Each of the following bounds what the reported results support.
...
**No corpus deletion, and a frozen corpus.** The corpus is fixed after initial indexing, so results describe a static collection. Behaviour under incremental corpus growth, including index drift and embedding staleness, is untested and is left to future work.
